...
class OriginTable extends StatefulWidget {
  ...
  const OriginTable({
    super.key,
    this.onReload,
    required Future<void> Function(dynamic origin) onEdit,
  });
...
class OriginTableState extends State<OriginTable> {
  ...
  void _addOrigin() {
    showDialog(
      ...
      builder: (_) => OriginForm(onSave: () => loadOrigins()),
    );
  }

  /// 🔹 Editar origen
  void _editOrigin(Origin origin) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (_) => OriginForm(origin: origin, onSave: () => loadOrigins()),
    );
  }
                                      ...
                                      paginatedData.map((origin) {
                                        return DataRow(
                                          cells: [
                                            ...
                                            DataCell(Text(origin.name)),
                                            DataCell(Text(origin.description)),
                                            DataCell(
                                              Icon(
                                                origin.sync
                                                    ? Icons.cloud_done
